...
def format_data(f, pheno):
	...
	for aline, trait in zip(ped_file.readlines(), pheno):
		begin = aline[:10]
		data = aline[10:]
		data = data.replace('0', 'A A').replace('2', 'B B')
		data = re.sub('1', lambda m: randomcallback(), data)
		new_file.write(begin+str(trait)+' '+data+'\n')
	ped_file.close()
	new_file.close()
	os.remove(f+'_genomes.ped')
	os.rename(f+'temp_genomes.ped', f+'_genomes.ped')
